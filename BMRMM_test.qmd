...
bmrmm_test <- df_shots |>
  filter(team_id == 17) |>
  select(-c(team_id, time_elapsed)) |>
  mutate(game_id = as.numeric(as.factor(game_id)),
         past_event = lag(event)) |>
  select(game_id, athlete_id_1, shot_dist, past_event, event)

m1 <- BMRMM(bmrmm_test, num.cov = 2, simsize = 50)
```
