...
df_discrete_lv2022 <- discrete_times |>
  ...
  mutate(event = case_when(score_value == 0 ~ 0,
                           score_value == 2 ~ 2,
                           score_value == 3 ~ 3),
...
fits <- lapply(1:10, function(i) {
  set.seed(i)
  mod <- depmix(event ~ shot_dist + time_btwn_shots, nstates = 2, family=binomial(), ntimes = game_obslength_vec[[1]], data = binomial_shots)
