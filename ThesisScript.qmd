...
build_data_3states <- function(df, team, season){
  shot_data |>
    ...
    select(game_id, game_play_number, event)
}
...
build_data_2states <- function(df, team, season){
  shot_data |>
    ...
    # CHANGED TO GAME_PLAY_NUMBER BECAUSE THESE ARE NON-OVERLAPPING
    # add a half second to non-unique time_elapsed values
    select(game_id, game_play_number, event)
}
...
# this doesn't work. numerical overflow in calculating likelihood.
# it works when there are 2 states, but not 3
# nevermind this doesn't work it doesn't converge
champs_2022.msm <- msm(event ~ game_play_number,
                       subject = game_id,
                       data = champs_2022,
                       qmatrix = Q,
                       gen.inits = T)

champs_2022_3states.msm <- msm(event ~ game_play_number,
                       subject = game_id,
...
## Try HiddenMarkov package

```{r}
# library(HiddenMarkov)
# Pi <- matrix(c(1/3, 1/3, 1/3,
#                1/3, 1/3, 1/3,
#                1/3, 1/3, 1/3), nrow = 3, byrow = T)
# delta <- c(1/3, 1/3, 1/3)
# 
# x <- dthmm(NULL, Pi, delta, "norm", list(mean = ))
```
...
```{r}
library(HMM)
hmm <- initHMM(States = c("State 1", "State 2"), Symbols = c("1", "2", "3"))
posterior_probs <- posterior(hmm, df_discrete$event)
# why doesn't this one work?
bw <- baumWelch(hmm, df_discrete$event, maxIterations=100, delta=1E-9, pseudoCount=0)
print(bw$hmm)
```
