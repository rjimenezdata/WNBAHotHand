...
summ_data <- shot_data |> 
  filter((team_id == 17 & season == 2022) | (team_id == 17 & season == 2023) | (team_id == 9 & season == 2024)) |>
  mutate(shot_type = as.factor(if_else(str_detect(text, "three point") | shot_dist > 22.146, 3, 2)),
...
summ_data |>
  ...
  summarize("Team" = unique(team_id),
            "Season" = unique(season),
    "Mean Shot Distance" = mean(shot_dist),
            "2-Pt Attempts" = sum(shot_type == 2),
    "3-Pt Attempts" = sum(shot_type == 3),
    "FG%" = mean(as.logical(scoring_play)*100),
    "Games Played" = n_distinct(game_id)
    ) |>
  mutate(Team = fct_recode(Team, "LV Aces" = "17", "NY Liberty" = "9")) |>
  ungroup() |>
  select(-c(team_id, season)) |>
  arrange(Season) |>
  gt() |>
  tab_style(
  ...
  tab_header(
    title = "Summary of Filtered Data"
  )
