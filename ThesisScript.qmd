...
fitmod <- fit(fits, emcontrol = em.control(random.start = TRUE, maxit = 2000, tol = 1e-8))

summary(fitmod)
mostlikelyseq <- posterior(fitmod)$state
with(rle(mostlikelyseq), tapply(lengths, values, FUN = max))
logLik(fitmod)


# RESULTS:
# negative log likelihood = -2226.701
# longest sequence of state 1: 344
# longest sequence of state 2: 2
# longest sequence of state 3: 2
fits2 <- depmix(event ~ shot_dist + athlete_id_1, nstates = 3, family=multinomial(), ntimes = game_obslength_vec[[1]], data = df_discrete_lv2022)

fitmod2 <- fit(fits2, emcontrol = em.control(random.start = TRUE, maxit = 2000, tol = 1e-8))

summary(fitmod2)
mostlikelyseq2 <- posterior(fitmod2)$state
with(rle(mostlikelyseq2), tapply(lengths, values, FUN = max))
logLik(fitmod2)
```
